...
export default function Topbar() {
  const { activeChain, switchChain } = useApp()
  const { theme, updateTheme } = useTheme()
  const [showChainMenu, setShowChainMenu] = useState(false)
  const [showSettings, setShowSettings] = useState(false)

  const currentChain = CHAINS[activeChain] || CHAINS.atom
  ...
  return (
    <header className="topbar">
      ...
      <div className="topbar-right">
        ...
        <button className="topbar-btn chain-btn" onClick={() => setShowChainMenu(!showChainMenu)}>
        ...
        {showChainMenu && (
          <div className="chain-menu">
            {Object.entries(CHAINS).map(([key, chain]) => (
              <button
                key={key}
                className={`chain-menu-item ${activeChain === key ? 'active' : ''}`}
                onClick={() => {
                  switchChain(key)
